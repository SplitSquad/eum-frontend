...
export const getShortTermForecast = async (params: {
  base_date: string; // 발표일자 (yyyyMMdd)
 ...
 */
export const convertWeatherCode = (category: string, value: string): string => {
  // 하늘상태(SKY) 코드 변환
  if (category === 'SKY') {
    const skyCodes: Record<string, string> = {
      '1': '맑음',
      '3': '구름많음',
      '4': '흐림',
    };
  ...
  // 강수형태(PTY) 코드 변환
  if (category === 'PTY') {
    const ptyCodes: Record<string, string> = {
      '0': '없음',
      '1': '비',
      '2': '비/눈',
      '3': '눈',
 ...
 */
export const convertToWeatherGrid = (lat: number, lon: number): { nx: number; ny: number } => {
