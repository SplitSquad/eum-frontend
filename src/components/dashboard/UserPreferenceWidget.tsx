...
import { Paper, Box, Typography, Divider, Badge, Tooltip, Avatar, Chip, LinearProgress, CircularProgress, IconButton } from '@mui/material';
...
const UserPreferenceWidget: React.FC = () => {
  ...
  useEffect(() => {
    const loadPreferenceData = async () => {
      ...
      try {
        // 병렬로 커뮤니티와 토론 추천 API 호출
        const [communityResponse, debateResponse] = await Promise.all([
          // 커뮤니티 추천 API
        ...
        if (Object.keys(communityAnalysis).length > 0) {
          ...
          const topCategories = sortedPreferences.slice(0, 5).map((item) => {
            ...
            return {
              ...
              color: tagColors[item.name] || `#${Math.floor(Math.random() * 16777215).toString(16)}`
            };
          });
        ...
        // 3. 정보 분석 데이터 - 임시 더미 데이터 (추후 API 연동)
        const infoDummyData = {
          '여행': 0.65,
          '맛집': 0.55,
          '사진': 0.48,
    ...
    { id: 'shopping', name: '쇼핑', percent: 5, color: '#2196f3' },
        ...
        setInfoPreferences({
          categoryData: [
            { id: 'travel', name: '여행', percent: 45, color: '#f44336' },
            { id: 'food', name: '맛집', percent: 25, color: '#ff9800' },
            { id: 'photo', name: '사진', percent: 15, color: '#9c27b0' },
            { id: 'history', name: '역사', percent: 10, color: '#4caf50' },
            { id: 'music', name: '음악', percent: 5, color: '#2196f3' },
          ],
          recommendedKeywords: [
            { id: 'europe', name: '유럽', weight: 9 },
            { id: 'jeju', name: '제주도', weight: 8 },
            { id: 'camera', name: '카메라', weight: 7 },
            { id: 'ancient', name: '고대사', weight: 6 },
            { id: 'jazz', name: '재즈', weight: 5 },
            { id: 'architecture', name: '건축', weight: 4 },
          ]
        });
