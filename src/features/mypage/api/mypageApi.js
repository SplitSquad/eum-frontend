...
class MypageApi {
    ...
    async getProfileInfo(userId) {
        try {
            ...
            // 사용자 선호도 정보 가져오기
            const preferenceResponse = await apiClient.get('/users/preference', {
                headers: {
                    'Authorization': token
                }
            });
            ...
            const preferenceData = preferenceResponse;
            let onBoardingData = {};
            if (preferenceData.onBoardingPreference) {
                try {
                    ...
                    if (typeof preferenceData.onBoardingPreference === 'string') {
                        ...
                        // 이미 객체면 그대로 사용
                        onBoardingData = preferenceData.onBoardingPreference;
                    }
                }
                catch (e) {
            ...
            const userProfile = {
                ...
                introduction: onBoardingData.introduction || '', // 온보딩 데이터에서 소개 가져오기
     ...
     * 내가 작성한 게시글 목록 조회
     */
    async getMyPosts(userId, page = 0, size = 10) {
        try {
            if (!userId) {
     ...
     * 내가 작성한 댓글 목록 조회
     */
    async getMyComments(userId, page = 0, size = 10) {
        try {
            if (!userId) {
            ...
            const response = await apiClient.get('/community/comment/written', {
                params: { userId, page, size }
            });
     ...
     * 내가 투표한 토론 목록 조회
     */
    async getMyDebates(userId, page = 0, size = 10) {
        try {
            if (!userId) {
            ...
            const mappedDebates = debates.map((debate) => ({
     ...
     */
    getEmptyPaginatedResponse(page, size) {
        return {
            content: [],
            pageable: {
                pageNumber: page,
