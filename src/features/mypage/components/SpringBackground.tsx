...
const PetalElement = styled.div<PetalProps>`
  position: absolute;
...
const CherryPetal = styled.div<FallingPetalProps>`
  position: absolute;
  top: -10%;
...
interface FallingPetalProps extends PetalProps {
  duration: number;
  delay: number;
}

const Wave = styled.div<WaveEffectProps>`
  position: absolute;
  bottom: 0;
  left: 0;
  width: 200%;
...
// 웨이브 이펙트 컴포넌트
const WaveEffect: React.FC<WaveEffectProps> = ({ delay, intensity }) => {
  return <Wave delay={delay} intensity={intensity} />;
};
...
const SpringBackground: React.FC<SpringBackgroundProps> = ({ 
  ...
  useEffect(() => {
    ...
    const initialPetals = Array.from({ length: petalCount }).map((_, i) => (
      <CherryPetal
        key={`initial-petal-${i}`}
