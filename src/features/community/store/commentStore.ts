...
interface CommentActions {
  ...
  // 추가된 댓글 관련 액션
  addCommentReaction: (postId: number, commentId: number, option: ReactionType) => Promise<void>;
    ...
    (set, get) => ({
      ...
      fetchComments: async (postId: number, page: number = 0, size: number = 10) => {
        try {
          ...
          // API 호출
          const response = await commentApi.CommentApi.getComments(
            numericPostId,
            'post',
            page,
            size
      ...
      createComment: async (postId: number, content: string, parentId?: number): Promise<void> => {
        try {
          ...
          const newComment = await commentApi.CommentApi.createComment(
            postId,
            'post',
            content.trim()
          );
      ...
      likeComment: async (commentId: number) => {
        try {
          ...
          // 실제 API 호출
          const response = await commentApi.CommentApi.reactToComment(
            commentId,
      ...
      createReply: async (postId: number, commentId: number, content: string): Promise<void> => {
        try {
          ...
          // API 호출
          const newReply = await commentApi.CommentApi.createReply(
            postId,
            commentId,
            content.trim()
          );
