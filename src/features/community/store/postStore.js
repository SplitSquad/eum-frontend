...
const setupLanguageChangeListener = () => {
    ...
    useLanguageStore.subscribe(state => {
        ...
        if (currentLanguage !== previousLanguage) {
            console.log(`[INFO] 언어가 변경됨: ${previousLanguage} → ${currentLanguage}, 게시글 데이터 새로고침`);
            ...
            setTimeout(() => {
                ...
                if (currentPost) {
                    console.log(`[INFO] 언어 변경으로 현재 게시글(ID: ${currentPost.postId}) 상세 정보 새로고침`);
...
export const usePostStore = create()(devtools((set, get) => ({
    // 게시글 관련 상태 초기값
    posts: [],
    currentPost: null,
    topPosts: [],
    ...
    fetchPosts: async (filter) => {
        try {
            ...
            // 검색 활성화된 경우, 검색 상태 유지하고 페이지 이동만 처리
                ...
                console.log('[DEBUG] 검색 상태에서 페이지 이동:', currentState.searchTerm, currentState.searchType);
                // 검색 API로 페이지 이동
            ...
            const mergedFilter = {
                ...
                postType: filter?.postType !== undefined
                    ? filter.postType
                    : currentState.postFilter.postType || '전체',
            };
            // 더 자세한 필터 로깅 추가
            ...
            const postTypeForCache = !mergedFilter.postType || mergedFilter.postType === 'ALL'
                ? '전체'
            ...
            const fetchData = async () => {
                try {
                    ...
                    let response;
                    try {
                        if (apiParams.postType === '전체')
                            apiParams.postType = undefined;
    ...
    createPost: async (postDto, files) => {
        ...
        try {
            if (postDto.postType === '전체')
                postDto.postType = undefined;
    ...
    updatePost: async (postId, postDto, files) => {
        ...
        try {
            ...
            if (postDto.postType === '전체')
                postDto.postType = undefined;
            const updatedPost = await postApi.PostApi.updatePost(postId, postDto, files);
            ...
            set(state => {
                ...
                const updatedPosts = state.posts.map(post => post.postId === postId
                    ? {
                        ...post,
    ...
    reactToPost: async (postId, option) => {
        try {
            ...
            if (currentPost && currentPost.postId === postId) {
                const response = await postApi.PostApi.reactToPost(postId, option);
                // 게시글 반응 상태 업데이트
                set({
                    currentPost: {
                        ...currentPost,
    ...
    searchPosts: async (keywordOrOptions, searchType, options) => {
        try {
            let keyword = '';
            ...
            set({
                ...
                postFilter: {
                    ...
                    location: searchOptions.region !== undefined
                        ? searchOptions.region
                        : currentState.postFilter.location,
                    postType: searchOptions.postType !== undefined
