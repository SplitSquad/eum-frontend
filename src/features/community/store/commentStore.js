// import { Comment, Reply, ReactionType } from '../types-folder/index'; // 타입 import 불가로 임시 주석 처리
import { create } from 'zustand';
import { devtools } from 'zustand/middleware';
import * as commentApi from '../api/commentApi';
// 댓글 관련 상태 관리 스토어
export const useCommentStore = create()(devtools((set, get) => ({
    // 댓글 관련 상태 초기값
    comments: [],
    ...
    replies: {},
    // 액션: 댓글 관련
    fetchComments: async (postId, page = 0, size = 10) => {
        try {
    ...
    getCommentReplies: async (commentId) => {
        try {
            ...
            if (response && response.replyList) {
                ...
                const processedReplies = response.replyList.map((reply) => {
    ...
    likeComment: async (commentId) => {
        try {
            set({ commentLoading: true, commentError: null });
            // 실제 API 호출
            const response = await commentApi.CommentApi.reactToComment(commentId, 'LIKE');
            // 댓글 좋아요 후 현재 댓글 업데이트
            ...
            set({ commentError: error.message, commentLoading: false });
        }
    },
    dislikeComment: async (commentId) => {
        try {
            set({ commentLoading: true, commentError: null });
            // 실제 API 호출
            const response = await commentApi.CommentApi.reactToComment(commentId, 'DISLIKE');
            // 댓글 싫어요 후 현재 댓글 업데이트
            const comments = [...get().comments];
            const index = comments.findIndex(c => c.commentId === commentId);
            if (index !== -1) {
        ...
        try {
            ...
            if (newReply) {
                ...
                const enhancedReply = {
                    ...
                    likeCount: typeof newReply.liked === 'number' ? Number(newReply.liked) : 0,
                    dislikeCount: typeof newReply.disliked === 'number' ? Number(newReply.disliked) : 0,
