...
const RegionSelector: React.FC<RegionSelectorProps> = ({ selectedRegion, onChange }) => {
  ...
  // 대분류 지역 변경 핸들러
  const handleMainRegionChange = (event: SelectChangeEvent<string>) => {
    const newMainRegion = event.target.value;
    onChange(newMainRegion === '전체' ? '전체' : `${newMainRegion}`);
  };

  // 소분류 지역 변경 핸들러
  const handleSubRegionChange = (event: SelectChangeEvent<string>) => {
    ...
    <Box sx={{ mb: 2 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 2 }}>
        ...
        <FormControl size="small" sx={{ minWidth: 120, flex: 1 }}>
          <InputLabel id="main-region-label">지역</InputLabel>
          <Select
            labelId="main-region-label"
            value={mainRegion}
            label="지역"
        ...
        {/* 소분류 선택 (대분류가 '전체'가 아닐 때만 표시) */}
        {mainRegion !== '전체' && (
          <FormControl size="small" sx={{ minWidth: 120, flex: 1 }}>
            <InputLabel id="sub-region-label">세부 지역</InputLabel>
            <Select
            ...
            >
              <MenuItem value="전체">{mainRegion} 선택하세요</MenuItem>
