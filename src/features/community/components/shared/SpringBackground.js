...
const WaveEffect = styled.div `
  ...
  z-index: 0;
`;
const SpringBackground = ({ children, noPadding = false }) => {
    ...
    return (_jsxs(SpringContainer, { noPadding: noPadding, children: [petalElements.map((petal, index) => (_jsx(PetalElement, { size: petal.size, top: petal.top, left: petal.left, delay: petal.delay }, index))), fallingPetals.map(petal => (_jsx(FallingPetal, { size: petal.size, left: petal.left, duration: petal.duration, delay: petal.delay }, petal.id))), _jsx(WaveEffect, {}), _jsx(Box, { sx: {
                    position: 'relative',
                    zIndex: 10,
                    flexGrow: 1,
                    width: '100%',
