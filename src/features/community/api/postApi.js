...
export const PostApi = {
    ...
    getPosts: async (params) => {
        try {
            ...
            console.log(`[DEBUG] 게시글 목록 처리 완료: ${posts.length}개 게시글, 총 ${total}개, 총 페이지: ${totalPages}`);
            const result = {
                postList: posts,
                total: total,
     ...
     * 인기 게시글 조회
     */
    getTopPosts: async (count = 5) => {
        try {
            // 추천 게시글 API 사용
            const response = await apiClient.get(`${BASE_URL}/recommendation`, {
     ...
     */
    searchPosts: async (keyword, searchBy = 'title_content', options = {}) => {
        try {
    ...
    getPostById: async (postId) => {
        try {
            const response = await apiClient.get(`${BASE_URL}/${postId}`);
            ...
            return {
                ...
                // 추가된 필드
                postType: typeof response.postType === 'string' ? toPostType(response.postType) : response.postType,
    ...
    createPost: async (postDto, files = []) => {
        try {
            ...
            // 3) API 요청
            return await apiClient.post(BASE_URL, formData, {
                headers: {
    ...
    updatePost: async (postId, postDto, files = []) => {
        try {
            ...
            // 3) API 요청
            return await apiClient.patch(`${BASE_URL}/${postId}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
    ...
    reactToPost: async (postId, option) => {
        try {
            ...
            const response = await apiClient.post(`${BASE_URL}/emotion/${postId}`, { emotion: emotionMapping[option] });
            return response;
        }
        catch (error) {
     ...
     * 내가 작성한 게시글 조회
     */
    getMyPosts: async (userId, page = 0, size = 6) => {
        try {
            const response = await apiClient.get(`${BASE_URL}/written`, {
