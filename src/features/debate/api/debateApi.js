...
import { mapDebateResToFrontend, ReactionType, } from '../types';
/**
 * 토론 관련 API
 */
const DebateApi = {
    ...
    getDebates: async (params) => {
        try {
            ...
            if (keyword && keyword.trim() !== '') {
                response = await apiClient.get(`/debate/search`, {
                    params: {
                ...
                response = await apiClient.get(`/debate`, {
                    params: {
                        page: page - 1, // Spring Boot는 0부터 시작하는 페이지 인덱스 사용
                        size,
                        sort,
            ...
            if (responseData.content && Array.isArray(responseData.content)) {
                const debates = responseData.content.map((item) => {
                    ...
                    if (item.nationPercent && typeof item.nationPercent === 'object') {
                        const countryStats = Object.entries(item.nationPercent).map(([countryCode, percentage]) => {
                            ...
                            return {
                                countryCode,
                                countryName,
                                count: Math.round((percentage / 100) * (item.voteCnt || 0)),
                ...
                console.log('변환된 debates 목록:', debates.map((d) => ({ id: d.id, title: d.title, category: d.category })));
                return {
                    debates,
            ...
            // {total: number, debateList: Array} 형식의 응답 (빈 배열 포함)
                ...
                if (Array.isArray(responseData.debateList)) {
                    const debates = responseData.debateList.map((item) => {
                        const debate = mapDebateResToFrontend(item);
                        // 카테고리 정보 유지 (중요: 항상 카테고리 정보를 유지해야 필터링 가능)
                        if (item.category) {
                            debate.category = item.category;
            ...
            // 직접 배열이 반환된 경우
                ...
                const debates = responseData.map((item) => {
                    const debate = mapDebateResToFrontend(item);
                    // 카테고리 정보 유지
                    if (item.category) {
                        debate.category = item.category;
     ...
     */
    getDebateById: async (debateId, includeComments = true) => {
        try {
            // 실제 API 연동
                ...
                if (data) {
                    ...
                    if (data.nationPercent && typeof data.nationPercent === 'object') {
                        ...
                        const countryStats = Object.entries(data.nationPercent).map(([countryCode, percentage]) => {
                            ...
                            return {
                                countryCode,
                                countryName,
                                count: Math.round((percentage / 100) * (data.voteCnt || 0)),
                                percentage: percentage,
                            };
                        });
                        debate.countryStats = countryStats;
                    ...
                    if (includeComments) {
                        try {
                            ...
                            const commentsResponse = await apiClient.get(`/debate/comment`, {
                                params: {
                                    debateId: debateId,
                                    sort: 'newest',
                                    page: 0,
                            ...
                            if (commentsResponse && commentsResponse.commentList) {
                                ...
                                // 댓글 데이터 처리
                                const comments = commentsResponse.commentList.map((comment) => {
                                ...
                                if (commentsWithReplies.length > 0) {
                                    ...
                                    // 병렬로 모든 댓글의 답글 가져오기
                                    await Promise.all(commentsWithReplies.map(async (comment) => {
                                        try {
                                            const replyResponse = await apiClient.get(`/debate/reply`, {
                                            ...
                                            if (replyResponse &&
                                                ...
                                                // 답글 데이터 변환 및 저장
                                                repliesMap[comment.id] = replyResponse.replyList.map((reply) => {
                                                    // 서버에서 제공하는 isState 값을 myReaction으로 변환
     ...
     * 조회수 증가 (백엔드에서 자동 처리됨)
     */
    increaseViewCount: async (debateId) => {
     ...
     * 찬반 투표하기
     */
    voteOnDebate: async (voteRequest) => {
        try {
            // 백엔드 API 요청 형식으로 변환
            const requestData = {
     ...
     * 감정표현 추가/수정
     */
    addReaction: async (reactionRequest) => {
        try {
     ...
     * 내가 투표한 토론 목록 조회
     */
    getVotedDebates: async (userId, page = 0, size = 5) => {
        try {
            ...
            // API 호출
            const response = await apiClient.get('/debate/voted', {
            ...
            if (response.content && Array.isArray(response.content)) {
                ...
                debates = response.content.map((item) => mapDebateResToFrontend(item));
            ...
            // debates 배열이 직접 있는 경우
                ...
                debates = response.debates.map((item) => mapDebateResToFrontend(item));
            ...
            // 직접 배열이 반환된 경우
                ...
                debates = response.map((item) => mapDebateResToFrontend(item));
                total = debates.length;
                totalPages = 1;
            }
            // 기타 응답 형식
    ...
    createDebate: async (data) => {
        try {
            const response = await apiClient.post('/debate', data);
    ...
    try {
        ...
        if (response) {
            ...
            if (data.todayDebateList && Array.isArray(data.todayDebateList)) {
                result.todayIssues = data.todayDebateList.map((item) => mapDebateResToFrontend(item));
            }
            // 인기 이슈 처리
            if (data.topDebate) {
...
export const getTodayIssues = async () => {
    try {
        ...
        if (response) {
            ...
            if (data.todayDebateList && Array.isArray(data.todayDebateList)) {
                return data.todayDebateList.map((item) => mapDebateResToFrontend(item));
            }
            // 이전 형식과의 호환성 유지
            if (Array.isArray(data)) {
                return data.map((item) => mapDebateResToFrontend(item));
            }
            // 단일 항목인 경우
            if (data.debateId) {
...
export const getHotIssue = async () => {
    try {
        ...
        if (response) {
            ...
            const popularResponse = await apiClient.get('/debate', {
...
export const getBalancedIssue = async () => {
    try {
        ...
        if (response) {
            ...
            const debatesResponse = await apiClient.get('/debate', {
...
export function convertIsStateToMyReaction(isState) {
    if (isState === '좋아요')
...
export function convertReactionTypeToIsState(reactionType) {
...
export const reactToComment = async (commentId, reactionType) => {
    try {
        ...
        // API 요청
        const response = await apiClient.post(`/debate/comment/${commentId}`, {
            emotion: emotionValue,
        });
 ...
 * 답글 리액션 추가/취소하기
 */
export const reactToReply = async (replyId, reactionType) => {
    try {
        console.log(`[DEBUG] 답글 반응 처리 시작 - 답글ID: ${replyId}, 타입: ${reactionType}`);
        // 백엔드 API 요청 형식 - 한글 문자열로 변환
        const emotionValue = convertReactionTypeToIsState(reactionType) || '좋아요';
        // API 요청
        const response = await apiClient.post(`/debate/reply/${replyId}`, {
            emotion: emotionValue,
        });
 ...
 */
export const getVotesByDebateId = async (debateId) => {
    try {
        // 새로 추가한 RESTful 엔드포인트 사용
        const response = await apiClient.get(`/debate/vote/${debateId}`);
