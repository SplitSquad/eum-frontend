...
import { Box, IconButton, Badge, Tooltip, ButtonGroup } from '@mui/material';
...
const ReactionButtons: React.FC<ReactionButtonsProps> = ({
  ...
  const handleReaction = async (reactionType: ReactionType) => {
    ...
    if (reactionType === ReactionType.LIKE || reactionType === ReactionType.DISLIKE) {
      ...
      const oppositeType =
        reactionType === ReactionType.LIKE ? ReactionType.DISLIKE : ReactionType.LIKE;
    ...
    // 기타 감정표현 (기쁨, 슬픔 등)은 기존대로 처리
      ...
      setLocalReactions(prev => ({
        ...
        [reactionType.toLowerCase()]:
          prev[reactionType.toLowerCase() as keyof typeof prev] + (newState ? 1 : -1),
      }));
    }
