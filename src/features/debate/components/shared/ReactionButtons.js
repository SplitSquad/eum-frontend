...
const mapIsStateToReactionType = (isState) => {
    if (!isState)
        return null;
    switch (isState) {
        case '좋아요':
        ...
        case '음...':
        case '글쎄요':
            return ReactionType.UNSURE;
        default:
            return null;
    }
};
const ReactionButtons = ({ targetId, targetType, reactions, isState, size = 'md', }) => {
    // 로컬 상태 관리
    ...
    const [userLikeState, setUserLikeState] = useState(false);
    const [userDislikeState, setUserDislikeState] = useState(false);
    const [userHappyState, setUserHappyState] = useState(false);
    const [userAngryState, setUserAngryState] = useState(false);
    const [userSadState, setUserSadState] = useState(false);
    const [userUnsureState, setUserUnsureState] = useState(false);
    // 실제 서버에서 받은 데이터 저장
    ...
    // 특정 반응의 상태 가져오기
    const getReactionState = (reactionType) => {
        switch (reactionType) {
            case ReactionType.LIKE:
    ...
    // 특정 반응의 상태 설정하기
    const setReactionState = (reactionType, state) => {
        switch (reactionType) {
            case ReactionType.LIKE:
    ...
    const handleReaction = async (reactionType) => {
        ...
        if (reactionType === ReactionType.LIKE || reactionType === ReactionType.DISLIKE) {
            ...
            if (currentState) {
                ...
                setLocalReactions(prev => ({
                    ...prev,
                    [reactionType.toLowerCase()]: prev[reactionType.toLowerCase()] - 1,
                }));
            }
        ...
        // 기타 감정표현 (기쁨, 슬픔 등)은 기존대로 처리
            ...
            setLocalReactions(prev => ({
                ...prev,
                [reactionType.toLowerCase()]: prev[reactionType.toLowerCase()] + (newState ? 1 : -1),
            }));
        }
        // 타입별로 다른 API 호출
        try {
    ...
    return (_jsxs(Box, { sx: { display: 'flex', flexWrap: 'wrap', gap: 1 }, children: [_jsx(Tooltip, { title: userDislikeState ? '싫어요를 취소한 후 누를 수 있습니다' : '좋아요', children: _jsx("span", { children: _jsx(Badge, { badgeContent: mergedReactions.like, color: "primary", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.LIKE), sx: {
                                color: userLikeState ? 'primary.main' : 'text.secondary',
                                padding: size === 'sm' ? 0.5 : 1,
                            }, disabled: userDislikeState, children: userLikeState ? (_jsx(ThumbUpIcon, { fontSize: getIconSize() })) : (_jsx(ThumbUpOutlinedIcon, { fontSize: getIconSize() })) }) }) }) }), _jsx(Tooltip, { title: userLikeState ? '좋아요를 취소한 후 누를 수 있습니다' : '싫어요', children: _jsx("span", { children: _jsx(Badge, { badgeContent: mergedReactions.dislike, color: "error", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.DISLIKE), sx: {
                                color: userDislikeState ? 'error.main' : 'text.secondary',
                                padding: size === 'sm' ? 0.5 : 1,
                            }, disabled: userLikeState, children: userDislikeState ? (_jsx(ThumbDownIcon, { fontSize: getIconSize() })) : (_jsx(ThumbDownOutlinedIcon, { fontSize: getIconSize() })) }) }) }) }), targetType === 'debate' && (_jsxs(_Fragment, { children: [_jsx(Tooltip, { title: "\uAE30\uC068", children: _jsx(Badge, { badgeContent: mergedReactions.happy, color: "success", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.HAPPY), sx: {
                                    color: userHappyState ? 'success.main' : 'text.secondary',
                                    padding: size === 'sm' ? 0.5 : 1,
                                }, children: _jsx(MoodIcon, { fontSize: getIconSize() }) }) }) }), _jsx(Tooltip, { title: "\uD654\uB0A8", children: _jsx(Badge, { badgeContent: mergedReactions.angry, color: "warning", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.ANGRY), sx: {
                                    color: userAngryState ? 'warning.main' : 'text.secondary',
                                    padding: size === 'sm' ? 0.5 : 1,
                                }, children: _jsx(MoodBadIcon, { fontSize: getIconSize() }) }) }) }), _jsx(Tooltip, { title: "\uC2AC\uD514", children: _jsx(Badge, { badgeContent: mergedReactions.sad, color: "info", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.SAD), sx: {
                                    color: userSadState ? 'info.main' : 'text.secondary',
                                    padding: size === 'sm' ? 0.5 : 1,
                                }, children: _jsx(SentimentVeryDissatisfiedIcon, { fontSize: getIconSize() }) }) }) }), _jsx(Tooltip, { title: "\uC74C...", children: _jsx(Badge, { badgeContent: mergedReactions.unsure, color: "default", showZero: false, sx: { '& .MuiBadge-badge': { fontSize: size === 'sm' ? '0.7rem' : '0.8rem' } }, children: _jsx(IconButton, { size: getIconSize(), onClick: () => handleReaction(ReactionType.UNSURE), sx: {
                                    color: userUnsureState ? 'text.primary' : 'text.secondary',
                                    padding: size === 'sm' ? 0.5 : 1,
                                }, children: _jsx(SentimentNeutralIcon, { fontSize: getIconSize() }) }) }) })] }))] }));
};
