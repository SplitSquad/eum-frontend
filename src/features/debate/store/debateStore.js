...
import DebateApi, { getTodayIssues, getHotIssue, getBalancedIssue, getSpecialIssues, } from '../api/debateApi';
import CommentApi from '../api/commentApi';
import { handleApiError } from '../utils/errorHandler';
export const useDebateStore = create()(devtools(persist((set, get) => ({
    // 초기 상태
    debates: [],
    totalDebates: 0,
    totalPages: 0,
    ...
    createComment: async (debateId, content, stance) => {
        try {
            ...
            if (comment) {
                // 임시 댓글을 실제 댓글로 교체
                set(state => {
                    const updatedComments = state.comments.map(c => c.id === tempComment.id ? comment : c);
                    // 임시 댓글이 목록에 없으면 추가
    ...
    updateComment: async (commentId, content) => {
        try {
            ...
            if (success) {
                // 성공 시 댓글 목록 업데이트
                set(state => {
                    const updatedComments = state.comments.map(comment => comment.id === commentId ? { ...comment, content } : comment);
                    return {
                        comments: updatedComments,
                        isLoading: false,
                    };
        ...
        try {
            ...
            if (success) {
                ...
                set(state => {
                    ...
                    // 현재 토론에 댓글 수 업데이트
                    const updatedCurrentDebate = state.currentDebate && commentToDelete
                        ? {
                            ...state.currentDebate,
    ...
    createReply: async (commentId, content) => {
        try {
            ...
            const updatedComments = get().comments.map(comment => comment.id === commentId
            ...
            try {
                ...
                if (reply) {
                    ...
                    if (updatedReplies[commentId]) {
                        updatedReplies[commentId] = updatedReplies[commentId].map(r => r.id === tempReply.id ? reply : r);
                        // 임시 대댓글이 목록에 없으면 추가
    ...
    deleteReply: async (replyId) => {
        try {
            ...
            if (success) {
                ...
                if (targetCommentId) {
                    const updatedReplies = { ...currentReplies };
                    updatedReplies[targetCommentId] = currentReplies[targetCommentId].filter(reply => reply.id !== replyId);
                    // 댓글 목록에서 해당 댓글의 대댓글 수 업데이트
