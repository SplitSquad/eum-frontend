...
import { getPreferredLanguage, isValidLanguage, SUPPORTED_LANGUAGE_CODES } from '../utils/languageUtils';

/**
 * 사용자 선호도 데이터 타입
 */
export interface UserPreferenceData {
  // 필수 필드 (백엔드 UserPreferenceReq와 일치)
  nation: string;
  language: string;
  gender: string;
  visitPurpose: string;
  period: string;
  onBoardingPreference: string; // JSON 문자열로 변환된 상세 온보딩 데이터
  isOnBoardDone: boolean;
}

/**
 * 온보딩 상세 데이터 타입 (JSON으로 저장됨)
 */
export interface OnboardingDetailData {
  ...
  age?: string;
  gender?: string;
  uiLanguage?: string; // 선호하는 UI 언어 (ko, en, jp 등)
  
  // 목적별 세부 정보 (유학, 여행, 취업, 거주 등)
  studyData?: Record<string, any>;
  travelData?: Record<string, any>;
  jobData?: Record<string, any>;
  livingData?: Record<string, any>;
...
export const getUserPreference = async () => {
  try {
    ...
    return await apiClient.get<UserPreferenceData>('/users/preference', config);
  } catch (error) {
 ...
 */
export const saveUserPreference = async (data: UserPreferenceData) => {
  try {
    // 로컬 스토리지에서 토큰 가져오기
    const token = localStorage.getItem('auth_token');
    ...
    return await apiClient.post<UserPreferenceData>('/users/preference', data, config);
  } catch (error) {
...
export const saveOnboardingData = async (
  purpose: string,
  detailData: OnboardingDetailData
) => {
