...
const ThemedAccordion = styled(Accordion) `
  ...
  margin-bottom: 8px;
  border: 1px solid 
    ${props => props.season === 'spring' ? 'rgba(255, 235, 235, 0.8)' :
  ...
  &.Mui-expanded {
    ...
    box-shadow: 0 6px 16px 
      ${props => props.season === 'spring' ? 'rgba(255, 170, 180, 0.15)' :
...
const ThemedSummary = styled(AccordionSummary) `
  ...
  background: linear-gradient(135deg, 
    ${props => props.season === 'spring' ? 'rgba(255, 245, 245, 0.7) 0%, rgba(255, 235, 235, 0.7) 100%' :
 ...
 */
const OnboardingTagSelector = ({ categories, tags, selectedTags = [], onChange, maxSelection = 5, title = '관심 태그 선택', description = '관심 있는 주제를 선택해주세요.', grouped = false, groupMapping, }) => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    const { season } = useThemeStore();
    const [expanded, setExpanded] = useState(categories.length > 0 ? categories[0].id : false);
    ...
    const renderGroupedTags = () => {
        return categories.map(category => (_jsxs(ThemedAccordion, { expanded: expanded === category.id, onChange: handleAccordionChange(category.id), season: season, children: [_jsx(ThemedSummary, { expandIcon: _jsx(ExpandMoreIcon, { sx: { color: primaryColor } }), season: season, children: _jsxs(Typography, { variant: "subtitle1", sx: {
                            fontWeight: 600,
                            color: '#555',
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            flexWrap: 'wrap',
                            gap: 0.5,
                        }, children: groupMapping && groupMapping[category.id]?.map(tagId => {
                            const tag = tags.find(t => t.id === tagId);
                            if (!tag)
                                return null;
                            return (_jsx(ThemedChip, { label: tag.name, clickable: true, season: season, selected: selectedTags.includes(tag.id), onClick: () => handleTagToggle(tag.id), size: isMobile ? 'small' : 'medium' }, tag.id));
                        }) }) })] }, category.id)));
    };
    // 태그를 그룹화 없이 일괄 표시
    const renderAllTags = () => {
        return (_jsx(Box, { sx: {
                ...
                gap: 0.5,
                mt: 2,
            }, children: tags.map(tag => (_jsx(ThemedChip, { label: tag.name, clickable: true, season: season, selected: selectedTags.includes(tag.id), onClick: () => handleTagToggle(tag.id), size: isMobile ? 'small' : 'medium' }, tag.id))) }));
    };
    return (_jsxs(Box, { sx: { width: '100%' }, children: [title && (_jsx(Typography, { variant: "h6", sx: { mb: 1, fontWeight: 600 }, children: title })), description && (_jsx(Typography, { variant: "body2", sx: { mb: 2, color: 'text.secondary' }, children: description })), maxSelection > 0 && (_jsxs(Typography, { variant: "body2", sx: {
                    mb: 1,
                    color: '#888',
                    fontSize: '0.85rem',
                    textAlign: 'right',
                }, children: ["\uD0DC\uADF8\uB294 \uCD5C\uB300 ", maxSelection, "\uAC1C\uAE4C\uC9C0 \uC120\uD0DD \uAC00\uB2A5\uD569\uB2C8\uB2E4 (", selectedTags.length, "/", maxSelection, ")"] })), grouped && groupMapping ? renderGroupedTags() : renderAllTags()] }));
};
