...
// 은하수 레이어 생성 함수
const createGalaxyLayers = (count) => {
    return Array.from({ length: count }).map((_, index) => ({
        id: index,
...
const AutumnBackground = ({ children, noPadding = false }) => {
    // 별 생성
    ...
    return (_jsxs(AutumnContainer, { noPadding: noPadding, children: [stars.map(star => (_jsx(Star, { size: star.size, top: star.top, left: star.left, opacity: star.opacity, twinkle: star.twinkle }, star.id))), galaxyLayers.map(layer => (_jsx(GalaxyMist, { top: layer.top, opacity: layer.opacity, size: layer.size }, layer.id))), _jsx(Moon, {}), fireflies.map(firefly => (_jsx(Firefly, { size: firefly.size, top: firefly.top, left: firefly.left, duration: firefly.duration, delay: firefly.delay, color: firefly.color }, firefly.id))), _jsx(Box, { sx: {
                    position: 'relative',
                    zIndex: 10,
                    flexGrow: 1,
                    width: '100%',
