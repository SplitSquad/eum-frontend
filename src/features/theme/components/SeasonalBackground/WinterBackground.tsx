...
// 중간 눈송이 컴포넌트
const MediumSnowflake = styled.div<{ size: number; left: string; delay: number; duration: number }>`
  position: absolute;
  top: -30px;
  width: ${props => props.size}px;
  height: ${props => props.size}px;
...
// 작은 눈송이 컴포넌트
const SmallSnowflake = styled.div<{ size: number; left: string; delay: number; duration: number }>`
  position: absolute;
  top: -20px;
  width: ${props => props.size}px;
  height: ${props => props.size}px;
...
// 별 요소
const Star = styled.div<{ size: number; top: string; left: string; opacity: number }>`
  position: absolute;
  width: ${props => props.size}px;
  height: ${props => props.size}px;
  top: ${props => props.top};
...
// 큰 눈송이 생성 함수
const createLargeSnowflakes = (count: number) => {
...
// 중간 눈송이 생성 함수
const createMediumSnowflakes = (count: number) => {
  return Array.from({ length: count }).map((_, index) => ({
    id: index,
...
// 작은 눈송이 생성 함수
const createSmallSnowflakes = (count: number) => {
  return Array.from({ length: count }).map((_, index) => ({
    id: index,
