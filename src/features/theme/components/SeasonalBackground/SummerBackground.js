...
const SummerBackground = ({ children, noPadding = false }) => {
    // 빗방울 생성 (양 감소)
    ...
    return (_jsxs(SummerContainer, { noPadding: noPadding, children: [clouds.map(cloud => (_jsx(CloudSilhouette, { width: cloud.width, height: cloud.height, top: cloud.top, left: cloud.left, opacity: cloud.opacity }, cloud.id))), raindrops.map(drop => (_jsx(Raindrop, { height: drop.height, left: drop.left, speed: drop.speed, delay: drop.delay, opacity: drop.opacity }, drop.id))), _jsx(RainpuddleContainer, { children: ripples.map(ripple => (_jsx(Ripple, { size: ripple.size, left: ripple.left, delay: ripple.delay }, ripple.id))) }), _jsx(Box, { sx: {
                    position: 'relative',
                    zIndex: 10,
                    flexGrow: 1,
                    width: '100%',
